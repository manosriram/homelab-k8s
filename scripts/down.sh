...
# within a specified directory using 'docker compose down'.

# Function to display usage instructions
usage() {
    echo "Usage: $0 <directory>"
    echo "Recursively finds docker-compose.yml files in the specified directory"
    echo "and runs them using 'docker compose down'."
    exit 1
}
...
find "$TARGET_DIR" -type f -name "docker-compose.yml" -print0 | while IFS= read -r -d $'\0' compose_file; do
    ...
    # This command will stop the services defined in the docker-compose.yml in the background
    if docker compose down; then
        echo "Successfully stopped services for $compose_dir"
    else
        echo "Error: Failed to stop services for $compose_dir"
    fi
